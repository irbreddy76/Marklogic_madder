...
<xsl:stylesheet exclude-result-prefixes="xsd functx an" version="2.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
    ...
    <!--<xsl:variable name="notice-details" as="element(an:abawd-notices)">
        <an:abawd-notices xmlns:an="info:md/dhr/abawd/notices#" xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance">
            ...
            <an:notice xsi:type="ChangeNotice">
                <an:nil/>
            </an:notice>
        </an:abawd-notices>
    </xsl:variable>-->
 ...
 <!-- Default template.  The default is not to output anything unless it has been specifically called. -->
        ...
        <fo:block text-align="center" space-before=".4in" font-weight="bold">
            <xsl:choose>
                ...
                <xsl:when test="$thislang eq 'am' ">
                    ...
                    <fo:block xml:lang="am"><fo:inline font-style="italic">የሁኔታ ለውጥ ማሳሰቢያ</fo:inline></fo:block>
                </xsl:when>
                <!-- Arabic 'ar' -->
                <xsl:when test="$thislang eq 'ar' ">
                    <fo:block-container xml:lang="ar" writing-mode="rl-tb">
                        <fo:block xml:lang="ar">إشعار تغير في الوضعية</fo:block>
                        <fo:block xml:lang="ar">الخاصة بالحصول على المستحقات من برنامج  الغذاء التكميلي</fo:block>
                    </fo:block-container>
                </xsl:when>
                <!-- Spanish 'es' -->
                <xsl:when test="$thislang eq 'es' ">
                    <fo:block><fo:inline font-style="italic">AVISO SOBRE CAMBIO DE SITUACIÓN DE BENEFICIOS DE SUPLEMENTOS ALIMENTICIOS</fo:inline></fo:block>                    
                </xsl:when>
                <xsl:otherwise> <!-- English 'en' is default -->
                    <fo:block><fo:inline font-style="italic">CHANGE OF STATUS NOTICE</fo:inline></fo:block>
    ...
    <xsl:template name="createBody-am">
        <fo:block space-before=".15in">ይህ ማሳሰቢያ እንዲሰጥዎት የተደረገበት ምክንያት ተጨማሪ የምግብ እርዳታዎችን ማግኘትዎን ለመቀጠል የተቀመጡትን የሥራ መስፈርቶች ማሟላት በአሁኑ ወቅት ስለሚመለከትዎ ነው። በፌደራል ህግ መሰረት ከዚህ በታች በዝርዝር የተገለጹት ሁኔታዎች እርስዎን የሚመለከቱ ካልሆኑ በስተቀር፤ ከጃንዋሪ 1, 2016 እስከ ዲሴምበር 31, 2018 ድረስ እነዚህን እርዳታዎች የሚያገኙት ለሶስት ወራት ብቻ እንዲሆን የተቀመጠው ገደብ ተፈጻሚ ይሆንብዎታል። እነዚህ ሁኔታዎችም፦</fo:block>
        ...
        <fo:block space-before=".15in">ከላይ ከተጠቀሱት ሁኔታዎች አንዱም ቢሆን እርስዎን የሚመለከት ከሆነ እባክዎአንድ ጊዜ  ያነጋግሩን። እንዲያ ካልሆነም በሳምንት ቢያንስ ለ20 ሰዓታት ያክል ሥራ የሚሰሩ ወይም ተቀባይነት ባለው የሥራ እንቅስቃሴ ውስጥ የሚሳተፉ ከሆኑ እነዚህን እርዳታዎች እንደገና  ማግኘት ይችላሉ። የሚሳተፉበት የሥራ እንቅስቃሴን እንዲያገኙ ልንረዳዎት እንችላለን። እባክዎን የጉዳይ ሥራ አስኪያጅዎን ያነጋግሩ።</fo:block>
    ...
    <xsl:template name="createBody-ar">
        <fo:block-container xml:lang="ar" writing-mode="rl-tb">
            <fo:block space-before=".15in" xml:lang="ar">أنت تتلقى هذا الإشعار لأنك تخضع لشروط العمل الخاصة بملفك للاستمرار في الحصول على مستحقاتك من برنامج الغذاء التكميلي . يقصر القانون الإتحادي أقصى مدة  للحصول على المستحقات لمدة لا تتجاوز ثلاثة أشهر في الفترة الواقعة ما بين الأول من كانون الثاني (يناير) 2016 إلى 31 كانون الأول (ديسمبر) 2018، إلا إذا كنت:</fo:block>
            <fo:list-block space-before=".15in" space-after=".15in">
                <fo:list-item space-after=".15in">
                    <fo:list-item-label start-indent="1em">
                        <fo:block>
                    ...
                    <fo:list-item-body start-indent="1.75em">
                        <fo:block xml:lang="ar">دون سن الثامنة عشر أو فوق سن 49 عاما</fo:block>
                    </fo:list-item-body>
                </fo:list-item>
                <fo:list-item space-after=".15in">
                    <fo:list-item-label start-indent="1em">
                ...
                <fo:list-item space-after=".15in">
                    ...
                    <fo:list-item-body start-indent="1.75em">
                        <fo:block xml:lang="ar">تشارك في برنامج علاج من الإدمان على المخدرات أو الكحول</fo:block>
                    </fo:list-item-body>
                </fo:list-item>
                <fo:list-item space-after=".15in">
                    <fo:list-item-label start-indent="1em">
                ...
                <fo:list-item space-after=".15in">
                    ...
                    <fo:list-item-body start-indent="1.75em">
                        <fo:block xml:lang="ar">	مسجل في مدرسة أو برنامج تدريبي أو مؤسيية تعليم عالي بنصف دوام على الأقل</fo:block>
                    </fo:list-item-body>
                </fo:list-item>
            </fo:list-block>
            <fo:block space-before=".15in" xml:lang="ar">إذا كنت تستوفي أيا من الشروط المذكورة أعلاه ، الرجاء الإتصال بنا فوراً . أما إذا لم تكن تستوفي أيا منها ، فيمكنك الاستمرار في تلقي المستحقات مرة أخرى إذا ما أعدت تقديم طلب وتقديم دليلاً  على أنك تعمل 20 ساعة في الأسبوع على الأقل ، أو تقدم ما يُثبت أنك قد سجلت في أحد "أنشـــطة القوى العاملة"  المعتمدة .</fo:block>
            <fo:block space-before=".15in" xml:lang="ar">يمكن الإطلاع على الأحكام الخاصة بمتطلب العمل هذا بالتفصيل في قانون الأحكام الإتحادية في 7CFR273. يمكن الإطلاع على المعلومات الخاصة بالمعاملة المعقولة وجلسات الاستماع العادلة على الصفحة التالية. بإمكاننا أن نساعدك في إيجاد نشاط عمل. الرجاء الاتصال بالمدير المشرف على ملفك. 
            </fo:block>
        </fo:block-container>
    </xsl:template>
    
    <xsl:template name="createBody-en">
        <fo:block space-before=".15in">You are receiving this notice because you are now subject to a work requirement in order to 
            continue to receive your Food Supplement benefits.  Federal law limits your benefits to three 
            months between January 1, 2016 and December 31, 2018 unless you are:</fo:block>
        <fo:list-block space-before=".15in" space-after=".15in">
            <fo:list-item space-after=".15in">
                <fo:list-item-label start-indent="1em">
                    <fo:block>
        ...
        <fo:block space-before=".15in">If you have any of the above conditions, please contact us at once.  If not, you can continue to receive benefits again if you work for pay for at least 20 hours per week or enroll in an approved work activity.  We can help you find a work activity.  Please contact your case manager.</fo:block>
    ...
    <xsl:template name="createBody-es">
        <fo:block space-before=".15in">Usted está recibiendo este aviso porque ahora está sujeto a un requisito de empleo para poder continuar recibiendo sus beneficios de Suplementos Alimenticios. Las leyes federales limitan sus beneficios a tres meses entre el 1 de enero de 2016 y el 31 de diciembre de 2018, a menos que usted:</fo:block>
        <fo:list-block space-before=".15in" space-after=".15in">
            <fo:list-item space-after=".15in">
                <fo:list-item-label start-indent="1em">
                    <fo:block>
            ...
            <fo:list-item space-after=".15in">
                ...
                <fo:list-item-body start-indent="1.75em">
                    <fo:block>Esté trabajando a cambio de paga durante por lo menos 20 horas semanales (30 horas semanales si trabaja por cuenta propia)</fo:block>
                </fo:list-item-body>
            </fo:list-item>
            <fo:list-item space-after=".15in">
                <fo:list-item-label start-indent="1em">
        ...
        <fo:block space-before=".15in">Por favor comuníquese con nosotros inmediatamente si usted cumple con alguna de las condiciones mencionadas arriba. De lo contrario, usted podrá recibir beneficios nuevamente si trabaja con remuneración por lo menos 20 horas semanales o se inscribe en una actividad laboral aprobada. Podemos ayudarle a encontrar una actividad laboral. Por favor comuníquese con el administrador de su caso.</fo:block>
