...
<xsl:stylesheet exclude-result-prefixes="xsd functx an" version="2.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
    ...
    <!-- <xsl:variable name="notice-details" as="element(an:abawd-notices)">
        <an:abawd-notices xmlns="info:md/dhr/abawd/notices#" xmlns:an="info:md/dhr/abawd/notices#" xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance" xsi:schemaLocation="info:md/dhr/abawd/notices# abawd-notice.xsd">
            ...
            <an:notice xsi:type="CaseClosureNotice">
                <an:case-closure-date>2016-05-05</an:case-closure-date>
            </an:notice>
        </an:abawd-notices>
    </xsl:variable> -->
 ...
 <!-- Default template.  The default is not to output anything unless it has been specifically called. -->
        ...
        <fo:block text-align="center" space-before=".4in" font-weight="bold">
            <xsl:choose>
                ...
                <xsl:when test="$thislang eq 'am' ">
                    ...
                    <fo:block xml:lang="am"><fo:inline font-style="italic">የጉዳይ መዘጋት ማሳሰቢያ</fo:inline></fo:block>
                </xsl:when>
                <!-- Arabic 'ar' -->
                <xsl:when test="$thislang eq 'ar' ">
                    <fo:block-container xml:lang="ar" writing-mode="rl-tb">
                        <fo:block xml:lang="ar">إشعار إغلاق الملف للحصول</fo:block>
                        <fo:block xml:lang="ar">على مستحقات من الغذاء التكميلي</fo:block>
                    </fo:block-container>
                </xsl:when>
                <!-- Spanish 'es' -->
                <xsl:when test="$thislang eq 'es' ">
                    <fo:block><fo:inline font-style="italic">DE BENEFICIOS AVISO DE DE SUPLEMENTOS CIERRE DE CASO ALIMENTICIOS</fo:inline></fo:block>
                </xsl:when>
                <xsl:otherwise> <!-- English 'en' is default -->
                    <fo:block><fo:inline font-style="italic">NOTICE OF CASE CLOSING</fo:inline></fo:block>
    ...
    <xsl:template name="createBody">
        <xsl:variable name="thisapplicationdate" select="$notice-details//an:case-closure-date/normalize-space() cast as xs:date"/>
        <xsl:variable name="thislang" select="$notice-details//an:clientLanguagePreferenceCode/normalize-space()"/>
    ...
    <xsl:template name="createBody-am">
        ...
        <fo:block space-before=".15in">ይህ ማሳሰቢያ እንዲሰጥዎት የተደረገበት ምክንያት የተጨማሪ ምግብ እርዳታዎችን ለእርስዎ መስጠታችንን ስለምናቋርጥ ነው። ጉዳይዎ በቀን <fo:inline font-weight="bold" text-decoration="underline"><fo:inline margin-right="8px">&#160;</fo:inline><xsl:value-of select="an:createApplicationDate($thisapplicationdate, $thislang)"/><fo:inline margin-left="8px">&#160;</fo:inline></fo:inline> ላይ ይዘጋል። እዚህ ውሳኔ ላይ ልንደርስ የቻልነውም ሥራ በመስራት ላይ ሳይሆኑ ወይም በሥራ እንቅስቃሴ ላይ ምንም ተሳትፎ ሳያደርጉ የተጨማሪ ምግብ እርዳታዎችን ለሶስት ወራት ያክል የተቀበሉ በመሆንዎት ነው። በፌደራል ህግ መሰረት እርዳታዎችን የሚያገኙት ለሶስት ወራት ብቻ ነው።</fo:block>
        ...
        <fo:block space-before=".15in">ከላይ ከተጠቀሱት ሁኔታዎች አንዱም ቢሆን እርስዎን የሚመለከት ከሆነ እባክዎአንድ ጊዜ ያነጋግሩን። እንዲያ ካልሆነም እንደገና ካመለከቱ እና በሳምንት ቢያንስ ለ20 ሰዓታት ያክል ሥራ የሚሰሩ ወይም ተቀባይነት ባለው የሥራ እንቅስቃሴ ውስጥ የሚሳተፉ ከሆኑ እነዚህን እርዳታዎች ማግኘት ይችላሉ።</fo:block>
    ...
    <xsl:template name="createBody-ar">
        ...
        <xsl:param name="thislang" required="yes"/>
        <fo:block-container xml:lang="ar" writing-mode="rl-tb">
            <fo:block space-before=".15in" xml:lang="ar">أنت تتلقى هذا الإشعار لأننا سوف نوقف المستحقات التي تحصل عليها من برنامج  الغذاء التكميلي . سوف يغلق ملف قضيتك بتاريخ <fo:inline font-weight="bold" text-decoration="underline"><fo:inline margin-right="8px">&#160;</fo:inline><xsl:value-of select="an:createApplicationDate($thisapplicationdate, $thislang)"/><fo:inline margin-left="8px">&#160;</fo:inline></fo:inline> . لقد اتخذنا هذا القرار لأنك تلقيت ثلاثة أشهر من المستحقات دون عمل أو دون المشاركة في أحد "أنشطة القوى العاملة . يقصر القانون الإتحادي مستحقاتك على مدة  لا تزيد عن ثلاثة أشهر ، إلا إذا كنت:</fo:block>
            <fo:list-block space-before=".15in" space-after=".15in">
                <fo:list-item space-after=".15in">
                    <fo:list-item-label start-indent="1em">
                        <fo:block>
            ...
            <fo:block space-before=".15in" xml:lang="ar">إذا كنت تستوفي أيا من الشروط المذكورة أعلاه ، الرجاء الإتصال بنا فوراً . أما إذا لم ينطبق عليك أياً  منها ، يمكنك تلقي المستحقات مرة أخرى إذا ما أعدت تقديم طلب وكنت تعمل 20 ساعة في الأسبوع على الأقل ، أو إذا ما سجلت في نشاط عمل معتمد.</fo:block>
    ...
    <xsl:template name="createBody-en">
        <xsl:param name="thisapplicationdate" required="yes"/>
        <xsl:param name="thislang" required="yes"/>
        <fo:block space-before=".15in">You are receiving this notice because we are stopping your Food Supplement benefits.   Your case will close on <fo:inline font-weight="bold" text-decoration="underline"><fo:inline margin-right="8px">&#160;</fo:inline><xsl:value-of select="an:createApplicationDate($thisapplicationdate, $thislang)"/><fo:inline margin-left="8px">&#160;</fo:inline></fo:inline>. We made this decision because you received three months of benefits without working or participating in a work activity.  Federal law limits your benefits to three months between January 1, 2016 and December 31, 2018 unless you are:</fo:block>
        <fo:list-block space-before=".15in" space-after=".15in">
            <fo:list-item space-after=".15in">
                <fo:list-item-label start-indent="1em">
                    <fo:block>
        ...
        <fo:block space-before=".15in">If you have any of the above conditions, please contact us at once.  If not, you can receive benefits again if you reapply and work at least 20 hours per week or enroll in an approved work activity.</fo:block>
    ...
    <xsl:template name="createBody-es">
        <xsl:param name="thisapplicationdate" required="yes"/>
        <xsl:param name="thislang" required="yes"/>
        <fo:block space-before=".15in">Usted está recibiendo este aviso porque vamos a cancelar sus beneficios de Suplementos Alimenticios. Su caso se cerrará el <fo:inline font-weight="bold" text-decoration="underline"><fo:inline margin-right="8px">&#160;</fo:inline><xsl:value-of select="an:createApplicationDate($thisapplicationdate, $thislang)"/><fo:inline margin-left="8px">&#160;</fo:inline></fo:inline> omamos esta decisión porque usted recibió tres.  meses de beneficios sin haber trabajado o participado en una actividad laboral. Las leyes federales limitan sus beneficios a tres meses, a menos que usted:</fo:block>
        <fo:list-block space-before=".15in" space-after=".15in">
            <fo:list-item space-after=".15in">
                <fo:list-item-label start-indent="1em">
                    <fo:block>
        ...
        <fo:block space-before=".15in">Por favor comuníquese con nosotros inmediatamente si usted cumple con alguna de las condiciones mencionadas arriba. De lo contrario, usted podrá recibir beneficios nuevamente si vuelve a solicitar y trabaja por lo menos 20 horas semanales o se inscribe en una actividad laboral aprobada.</fo:block>
