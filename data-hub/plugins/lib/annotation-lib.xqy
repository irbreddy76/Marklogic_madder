...
declare function an:annotateDocument($uri as xs:string?, $params as map:map) {
  ...
  (:This will be suffixed onto the annotation property name so that multiple annotations
    can be added to a document for a single annotation type :)
  let $annotation-type := for $property in json:array-values(map:get($params, "properties"))
    return
      if (map:get($property, "name") = "annotationType") then
    ...
    if(fn:empty($uri) or fn:string-length($uri) = 0) then
      ...
      let $_ :=
        if(fn:empty($identifier-query)) then fn:trace(" -- No URI and No Identifiers provided.  No Document to update.", $TRACE_LEVEL_DETAIL)
          ...
          let $_ := (
            map:put($content, "annotation-" || $annotation-type, $annotation), (:AGD :)
      ...
      return
        if(fn:doc-available($uri)) then (
          ...
          let $_ := (
            map:put($content, "annotation-" || $annotation-type, $annotation),
            map:put($doc, "content", $content)
          )
